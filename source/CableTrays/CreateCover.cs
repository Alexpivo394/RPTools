...
{
    private const string CoverFamilyName = "Крышка"; // Family name
    private const string CoverFamilyPath = @"Y:\12-BIM\Стандарт\Общие семейства и мануалы ко всем шаблонам\Семейства ЭОМ, СС\Соединители лотка\Крышка.rfa"; // Srver path
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        if (coverSymbol == null)
        {
            TaskDialog.Show(
                "Ошибка",
                $"Не удалось найти или загрузить семейство {CoverFamilyName}"
            );
        ...
        using (var trGr = new TransactionGroup(doc, "Добавление крышек"))
        {
            ...
            foreach (var tray in trays)
            {
                ...
                if (coverParam == null || coverParam.AsInteger() != 1)
                    continue;

                var width = tray.get_Parameter(BuiltInParameter.RBS_CABLETRAY_WIDTH_PARAM)
                    .AsDouble();
                var height = tray.get_Parameter(BuiltInParameter.RBS_CABLETRAY_HEIGHT_PARAM)
                    .AsDouble();
                ...
                FamilyInstance instance = doc.RunInTransaction(
                    "Создание крышек",
                    () =>
                ...
                // if (instance.LookupParameter(articulParamName).AsInteger() != 0)
                // {
                //     doc.RunInTransaction(
                //         "Обнуление артикула",
                //         () =>
                //         {
                //             instance.LookupParameter("ADSK_Код изделия").Set("");
                //             instance.LookupParameter("ADSK_Наименование").Set("");
                //         }
                //     );
                // }
    ...
    private FamilySymbol GetOrLoadFamilySymbol(Document doc, string familyName, string familyPath)
    {
        ...
        var loadedSymbol = new FilteredElementCollector(doc)
            .OfClass(typeof(FamilySymbol))
            .Cast<FamilySymbol>()
            .FirstOrDefault(f => f.FamilyName == family.Name);

        return loadedSymbol;
    }

    private void RotateCupByTrayNormal(
        Document doc,
        CableTray tray,
    ...
    )
    {
        ...
        var angel2 = trayNormal.AngleOnPlaneTo(cupNormal, trayDirection);
        var locPoint2 = famInstance.Location as LocationPoint;

        var axe2 = Line.CreateBound(locPoint2.Point, locPoint2.Point + normal2);

        doc.RunInTransaction(
            "Поворот",
            () => ElementTransformUtils.RotateElement(doc, famInstance.Id, axe2, -angel2)
        );
    }

    private void RotateCupOnTrayDirection(
        Document doc,
        CableTray tray,
        FamilyInstance famInstance,
        XYZ trayDirection
    )
    {
        ...
        doc.RunInTransaction(
            "Поворот",
            () => ElementTransformUtils.RotateElement(doc, famInstance.Id, axe, -angle)
        );
    }
}
