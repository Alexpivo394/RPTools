...
public sealed class TestProjectModule : Module
{
    protected override async Task ExecuteModuleAsync(IModuleContext context, CancellationToken cancellationToken)
    {
        ...
        foreach (var configuration in configurations)
        {
            await context.SubModule(configuration,
                async () => await TestAsync(context, configuration, cancellationToken));
        }
    }
    ...
    /// </summary>
    private static async Task<CommandResult> TestAsync(IModuleContext context, string configuration,
        CancellationToken cancellationToken)
    {
        return await context.DotNet().Test(new DotNetTestOptions
        {
            Solution = Solutions.ToadTools.FullName,
